...
$$
\begin{array}{cc}
\includegraphics[width=.5\textwidth]{NH.pdf}&\includegraphics[width=.5\textwidth]{RG.pdf}\\
h=NH&h=RG
\end{array}
$$
...
$$
\begin{array}{cc}
\includegraphics[width=.5\textwidth]{NH_1_12.pdf}&\includegraphics[width=.5\textwidth]{RG_1_12.pdf}\\
h=\NH&h=\RG
\end{array}
$$
...
\begin{figure}[!htbp]
\hspace{2cm}For all $h\in\{\text{NH}, \text{RG}\}$ maximize:
$$U_{h}$$
...
\todoVK{We have a problem here, how do we know that a solution always exists? Might need to change problem to not have target as a contraint but instead simply consider distance from constraint. The problem with that is that we won't necessarily have the existence theorem anymore... I need to figure this out.}
This game is equivalent to a bi matrix game with restriction to pure strategies where both players aim to get their utilisation as close to, without going past, a certain target.
...
The following result is a sufficient condition for the existence of equilibria:\\
...
\begin{figure}[!htbp]
$$\begin{array}{cc}
\includegraphics[width=.5\textwidth]{best_responses_ex_for_proof_NH.pdf}&
\includegraphics[width=.5\textwidth]{best_responses_ex_for_proof_RG.pdf}
\end{array}$$
...
\begin{figure}[!hbtp]
\begin{center}
\includegraphics[width=.7\textwidth]{proof.pdf}
\end{center}
\caption{Graphical proof of the fact that $f_{\NH}(K_{\RG})<f_{\NH}(K_{\RG}+1)$}\label{proof}
\end{figure}

Let $k^*=f(K_{\RG})$ for a given $K_{\RG}$.
As $K_{\RG}$ increases, the utilisation $U_{\NH}$ for $k^*$ will be non-increasing, but it will also be non-increasing for all $k<k^*$.
Thus, $f_{\NH}(K_{\RG})\geq k^*$: strict inequality depending on whether or not the utilisation for $K_{\NH}>k^*$ is below $t$.
\end{proof}
...
For example, in Figure \ref{Fig:best_responses_model1}a if RG chooses $K_{\RG}=6$, NH has best response $K_{\NH}=7$ .
Similarly, if NH has best response $K_{\NH}=3$, RG has best response $K_{\RG}=15$.
A Nash equilibrium for our game is a pair of points that intersect (in this case: $(K_{\NH}, K_{\RG})=(8,16)$ is a Nash equilibrium).

\begin{figure}[!htbp]
$$
\begin{array}{cc}
\includegraphics[width=.5\textwidth]{best_responses_model1.pdf}&
\includegraphics[width=.5\textwidth]{best_responses_model1_t=0_6.pdf}\\
...
For this model the Nash equilibrium is at $(8,16)$, which gives $\widetilde T=3.64$ and we obtain $T^*=3.64$.
Importantly, a PoA of 1 is not guaranteed for this problem.
For example in Figure \ref{Fig:best_responses_model1}b similar best response behaviour is shown for $t=0.6$ for which the PoA$=1.18$ (the optimal throughput is again at $(8,16)$).

Whilst removing central control, a certain influence can be exerted by a choice of $t$.
Figure \ref{Fig:target_demand_model1} shows the effect of $t$ and overall demand. Note, the non linear scale in \ref{Fig:target_demand_model1}a.
...
$$
\begin{array}{cc}
\includegraphics[width=.5\textwidth]{lnPoAmodel1targetvdemand.pdf}
&
\includegraphics[width=.5\textwidth]{model1targetvdemandfort-5.pdf}\\
\text{(a)}&\text{(b)}
\end{array}
$$
\caption{PoA for different target and demand rates (the second plot is a given region of the first)} \label{Fig:target_demand_model1}
\end{figure}

We see that an extremely large PoA is obtained for $t<0.2$. If we concentrate on a value of $t>0.5$ then the PoA is still extremely high: a PoA of 1.9 corresponds to 90\% less throughput of patients.
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=10cm]{argminPoAmodel1.pdf}
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=10cm]{model2targetvdemand.pdf}
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=12cm]{argminPoAmodel2.pdf}
...
In this work a generic game theoretical model has been presented that accounts for selfish actions of two CCUs.
...
A result is proved that allows for the assertion of existence of Nash equilibrium. This result is then applied to two particular models that are influenced by discussions with ABHB:
...
An analysis of the effect of selfish behaviour is given for both of these models in the form of PoA calculations.
The PoA is calculated so as to measure the effect of selfish behaviour on overall patient throughput.
...
Importantly a non negligible effect of selfish behaviour is calculated for certain policy target values.
...
Investigating demand is akin to investigating the capacity of the CCUs and as shown in Figure \ref{capacity}: if capacity is not sufficient selfish behaviour can have a very damaging effect on overall patient throughput.

\begin{figure}[!htbp]
$$\begin{array}{cc}
\includegraphics[width=.5\textwidth]{model1capacity.pdf}&
\includegraphics[width=.5\textwidth]{model2capacity.pdf}
\\
\text{a: Strict diversion}&\text{a: Soft diversion}\\
\end{array}$$
...
The code used in this work was written in Python and can be found here: \url{drvinceknight/measuring...}.
The graphics for this paper were obtained using \cite{Hunter:2007, sage} a worksheet with the data and code used for the plots can be found here: \url{https://sage.maths.cf.ac.uk/home/pub/120/}.
