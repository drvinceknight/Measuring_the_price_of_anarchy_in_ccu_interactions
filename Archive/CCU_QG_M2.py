...
Code to obtain the PoA by exhaustive search of best responses for model 2 of the paper.
...
def q(a,b,c,d,k1,k2,mu1,mu2,lmbda1,lmbda2,m,n):
    """
    Returns the probability of going from state (a,b) to state (c,d), assuming thresholds k1, k2. This is the transition rates for model 2.
    """
    if (a-c,b-d)==(1,0):
        return mu1*a
    if (a-c,b-d)==(0,1):
        return mu2*b
    if ((a < k1 and b < k2) or (a >= k1 and b >= k2)) and (a-c,b-d)==(-1,0):
        return lmbda1
    if ((a < k1 and b < k2) or (a >= k1 and b >= k2)) and (a-c,b-d)==(0,-1):
        return lmbda2
...
def PoA(m,n,mu1,mu2,lmbda1,lmbda2,target):
    ...
    bestrg={k1:0 for k1 in range(-1, m+1)}  # Initiate 0 best responses (start at -1 only to be able to obtain results quick)
    for k1 in range(m+1):  # Loop over all strategies for player 1
        devsqmin=100
        for k2 in range(bestrg[k1-1],n+1):  # Find the best response for player 2
    ...
    bestnh = {k1:0 for k1 in range(-1, n+1)}  # Initiate 0 best responses (start at -1 only to be able to obtain results quick)
    for k2 in range(n+1):  # Loop over all stratgies for player 2
        devsqmin=100
        for k1 in range(bestnh[k2-1], m+1):  # Find the best response for player 1
...
if __name__ == '__main__':
    ...
    target=t
    print PoA(m,n,mu1,mu2,lmbda1,lmbda2,target)
#    k1 = 2
#    k2 = 3
#    print P2(0,0,0,1,k1,k2,mu1,mu2,lmbda1,lmbda2)
#    print P2(2,2,2,3,k1,k2,mu1,mu2,lmbda1,lmbda2)
#    print P2(3,3,3,4,k1,k2,mu1,mu2,lmbda1,lmbda2)
#    Q = Transition_Rate_Matrix(m,n,k1,k2,mu1,mu2,lmbda1,lmbda2)
#    for row in Transition_Rate_Matrix(m,n,k1,k2,mu1,mu2,lmbda1,lmbda2):
#        print row
#    print Markov_Chain_Solver(Q,k1,k2,m,n,cache={})
#    print PoA(m,n,mu1,mu2,lmbda1,lmbda2,target)

#    from csv import writer
#    wrtr = writer(open('m2.csv','w'))
#    for row in Q:
#        wrtr.writerow(row)
