...
\begin{abstract}
Hospital throughput is often studied and optimised in isolation, ignoring the
interactions between hospitals.  In this paper Critical Care Unit (CCU)
interaction is placed within a game theoretic framework.  The methodology
involves the use of a normal form game underpinned by a two dimensional
continuous Markov chain.  A theorem is given that proves that a Nash Equilibrium
exists in pure strategies for the games considered.

In the United Kingdom a variety of utilisation targets are often discussed:
aiming to ensure that wards/hospitals operate at a given utilisation value.  The
effect of these target policies is investigated justifying their use to align
the interests of individual hospitals and social welfare.  In particular, we
identify the lowest value of a utilisation target that aligns these.
\end{abstract}
...
These policies are studied in the setting of a single hospital, and thus, from a game theoretic point of view correspond to rational behaviour (a good game theory reference text is \cite{Maschler2013}).
...
The CCU occupancy rates for some hospitals are reportedly very high
(\cite{Mitchell1995,Smith1995}) and a shortage of beds has been identified
...
This paper describes part of a project undertaken with managers from the Aneurin Bevan University Health Board (ABUHB), which is an NHS Wales organization in South Wales, that serves 21\% of the total population of Wales (\cite{Boarda}).
...
The paper is organised as follows: Section \ref{queueingandgamemodels} presents
the general methodology as well as a theoretical existence condition for Nash
Equilibrium. Section \ref{results} presents the findings from two models.
Finally, conclusions and further ideas for progression are given in Section
\ref{conclusions}.
...
The capacity thresholds are denoted as $K_{H}\in\mathbb{Z}$ for
$H\in\{\NH,\RG\}$. Note that $0\leq K_H\leq c_H$.
...
The state space for the Markov chain is given by (\ref{statespace}).
...
\begin{center}
    \begin{tikzpicture}[scale=.8, every node/.style={scale=0.8}]
        ...
        % Boundaries ------------------------------------
        % -----------------------------------------------
        \draw [dashed, red, line width=1mm, opacity=.3] (12,2) -- (12,-20);
        \draw [dashed, red, line width=1mm, opacity=.3] (-2,-12) -- (20,-12);
        ...
        \draw (bc) edge[out=-45,in=-135,<-,thick] node [below] {\tiny$3\mu_{\RG}$} (ellipsisb1);
        ...
        \draw (cc) edge[out=-45,in=-135,<-,thick] node [below] {\tiny$3\mu_{\RG}$} (ellipsisc1);
        ...
        \draw (dc) edge[out=-45,in=-135,<-,thick] node [below] {\tiny$3\mu_{\RG}$} (ellipsisd1);
        ...
        \draw (ec) edge[out=-45,in=-135,<-,thick] node [below] {\tiny$3\mu_{\RG}$} (ellipsise1);
        ...
        \draw (fc) edge[out=-45,in=-135,<-,thick] node [below] {\tiny$3\mu_{\RG}$} (ellipsisf1);
        ...
        \draw (zc) edge[out=-45,in=-135,<-,thick] node [below] {\tiny$3\mu_{\RG}$} (ellipsisz1);
...
The stochastic transition rate matrix $Q=Q(c_{NH},c_{RG})$ of the continuous-time Markov chain (\cite{Stewart2009}) has entries $q_{ij}=q_{(u_i,v_i),(u_j,v_j)}$ which is the rate at which a transition from state $i$ to state $j$ occurs. The transition rates are given by (\ref{transitions}) and are illustrated diagrammatically in Figure \ref{mc}.
...
\end{array}
$$
\caption{Steady state probabilities for each hospital ($H\in\{\NH,\RG\}$) with different threshold pairs ($(K_{\NH}, K_{\RG})=(6,12)$)} \label{steady_state_probs}
\end{figure}
...
\end{array}
$$
\caption{Steady state probabilities for each hospital ($H\in\{\NH,\RG\}$) with different threshold pairs ($(K_{\NH}, K_{\RG})=(1,12)$)} \label{steady_state_probs_1_12}
\end{figure}
...
The function $f_H$ is well defined as it maximises a continuous function over a
finite discrete set.  In case of multiple values that minimize $U_H$, it is
assumed that $f_H$ returns the lowest such value, this is consistent with the
Price of Anarchy (PoA) being a theoretical upper bound of the effect of
uncoordinated behaviour (\cite{TimRoughgarden}).

As such if $f_H$ is non-decreasing then it is in fact a stepwise non-decreasing
function.  If we consider $f_{\NH}$ and $f_{\RG}$ plotted on the same axis (so
that the domain of $f_{\NH}$ is the $x$-axis and the domain of $f_{\RG}$ is the
y-axis) it is obvious to see that the functions must intersect at some point as
shown in Figure \ref{exampleforproof}.

\begin{figure}[!htbp]
    \begin{center}
        \includegraphics[width=.7\textwidth]{./Images/best_responses_ex_for_proof.pdf}
    \end{center}
    \caption{Plots of best response functions: $f_{\NH}(K_{\RG})$ (crosses) and $f_{\RG}(K_{\NH})$ (diamonds)}\label{exampleforproof}
\end{figure}
    ...
    \begin{center}
        \begin{tikzpicture}[scale=.8, every node/.style={scale=0.8}]
            \coordinate (origin) at (0,0);
                ...
                \node at ($(origin) + (1,2.5)$) {\(\lambda_{\NH}^{(h,l)}\)};
                \node at ($(origin) + (3.5,4.5)$) {\(\lambda_{\NH}^{(h,h)}\)};
                % Effect of KRG
        ...
        \end{tikzpicture}
    \end{center}
    \caption{The effect of the threshold (\(K_{\RG}\)) on the effective arrival rate (\(\bar \lambda\)). The same behaviour is exhibited for \(\lambda_{\NH}^{(l,l)}\) and \(\lambda_{\NH}^{(l, h)}\).}\label{fig:effect_of_KRG_on_arrival_rate}
\end{figure}
...
In the same way (illustrated by Figure \ref{fig:effect_of_KNH_on_arrival_rate}),
this gives: \(U_{\NH}(K_{\NH}) = U_{\NH}(\bar \lambda_{\NH}(K_{\NH})) \leq
U_{\NH}(\bar \lambda_{\NH}(K_{\NH}+1)) = U_{\NH}(K_{\NH}+1)\) giving:
    ...
    \begin{center}
        \begin{tikzpicture}[scale=.8, every node/.style={scale=0.8}]
            \coordinate (origin) at (0,0);
                ...
                \node at ($(origin) + (1,3.5)$) {\(\lambda_{\NH}^{(h,l)}\)};
                \node at ($(origin) + (3.5,1.5)$) {\(\lambda_{\NH}^{(h,h)}\)};
                % Effect of KNH
        ...
        \end{tikzpicture}
    \end{center}
    \caption{The effect of the threshold (\(K_{\NH}\)) on the effective arrival rate (\(\bar \lambda\)). The same behaviour is exhibited for \(\lambda_{\NH}^{(l,l)}\) and \(\lambda_{\NH}^{(l, h)}\).}\label{fig:effect_of_KNH_on_arrival_rate}
\end{figure}
...
\begin{figure}[!hbtp]
    \begin{center}
        ...
        \end{tikzpicture}
    \end{center}
    \caption{Values of utilisation (\(U_{\NH}\)) and the corresponding inequalities for varying cutoff thresholds.}\label{fig:variation_of_UNH}
\end{figure}
...
Recalling equations
(\ref{equ:UNH_as_a_function_of_KRG}-\ref{equ:UNH_as_a_function_of_KNH}) and
Figure~\ref{fig:variation_of_UNH} gives:
...
The game theoretic model of (\ref{model1}) is solved using exhaustive
consideration of best responses whilst taking advantage of the structure
identified by the Lemma of Section~\ref{queueingandgamemodels}. For any given
pair of threshold strategies the matrix equation $\pi Q=0$ is solved by
obtaining a basis for the kernel of $Q$. For the purpose of this paper this is
implemented in Sagemath (\cite{sage}).
...
as a temporary overflow measure. This models corresponds to the first
...
\lambda_{\NH}, &\text{if }r=(l,l)\\
\lambda_{\NH}+\lambda_{\RG},  &\text{if }r=(l,h)\\
0, &\text{if }r\in\{(h,l),(h,h)\}\\
\end{cases}$$}
\end{minipage}
\begin{minipage}[t]{0.5\textwidth}
\small{$$\lambda_{\RG}^{(r)}=\begin{cases}
\lambda_{\RG}, &\text{if }r=(l,l)\\
\lambda_{\NH}+\lambda_{\RG},  &\text{if }r=(h,l)\\
0, &\text{if }r\in\{(l,h),(h,h)\}\\
\end{cases}$$}
\end{minipage}
...
The lowest value of $t$ which gives PoA$=1$ for the actual demand levels ($x=0$) is in fact $t=0.72$.
It is also noted that as demand increases the effect of uncoordinated behaviour increases (and the recommended target also increases) as shown in Figure \ref{mintargetvdemand}. This is potentially due to the fact that as demand increases there is the scope for larger discrepancy between optimal and sub optimal behaviours.
...
\begin{minipage}[t]{0.5\textwidth}
\small{$$\lambda_{NH}^{(r)}=\begin{cases}
\lambda_{NH}, &\text{if }r\in\{(l,l),(h,h)\}\\
\lambda_{NH}+\lambda_{RG},  &\text{if }r=(l,h)\\
0, &\text{if }r=(h,l)\\
\end{cases}$$}
\end{minipage}
\begin{minipage}[t]{0.5\textwidth}
\small{$$\lambda_{RG}^{(r)}=\begin{cases}
\lambda_{RG}, &\text{if }r\in\{(l,l), (h,h)\}\\
\lambda_{NH}+\lambda_{RG},  &\text{if }r=(h,l)\\
0, &\text{if }r=(l,h)\\
\end{cases}$$}
\end{minipage}
...
target.  When the demand is low, there is no scope for uncoordinated behaviour
to be damaging.  When the demand is very high, the system is saturated and once
again uncoordinated behaviour has no negative effect in comparison to optimal
behaviour.  There is however a region of demand for which there is a high PoA.
...
A result is proved that allows for the assertion of existence of a Nash
Equilibrium. This result is then applied to two particular models that are
influenced by discussions with a local health board. \textbf{Strict diversion}:
patients can be lost to the system if both CCUs declare being in diversion.
\textbf{Soft diversion}: if both CCUs are in diversion then they cannot divert
their own patients.
...
\begin{itemize}
    \item The assumptions as to the strategy space of our players is
        restrictive: a single threshold policy might not be optimal (although it
        is present in various pieces of literature on optimal control of
        queueing systems: \cite{naor1969regulation, shone2013comparisons}).
        Indeed, in reality critical care managers could have far more complex
        boundaries for their heuristic decision making;
    \item This model only assumes the presence of two players however in reality
        the system has a variety of stakeholders. Multiplayer systems could be
        worth considering. This would reflect health boards/hospitals in a
        concentrated area so that interactions are not just between two
        hospitals but between many.
    \item The restriction to pure strategies is influenced by discussions with
        ABUHB and also does not detract from the results presented thanks to the
        Theorem and Lemma of Section \ref{queueingandgamemodels}.  However,
        allowing for mixed strategies could also be of interest, corresponding
        to decision making that is not constant over time. Managers could
        alternate between a variety of behaviours: at time accepting patient
        despite being busy and at other times not;
    \item Patient length of stay is assumed to be dependent on the CCU at which
        they receive service. A further extension of the work would be to use
        the service rate from original CCU (prior to diversion). This would
        require a Markov chain with a higher dimensional state space. In
        practice this corresponds to patient morbidity corresponding to the
        original CCU.
\end{itemize}
...
Finally, congestion and throughput is not the only concern of a healthcare
system. Further work could involve the investigation of patient survival instead
of throughput as utility.  This would be similar to work such as
\cite{erkut2008ambulance, knight2012ambulance}.

The code used in this work can be found here: \url{https://gist.github.com/anonymous/81effc06eea70a9e4e2f}.
The graphics for this paper were obtained using software described in \cite{Hunter:2007, sage} a worksheet with the data and code used for the plots can be found here: \url{https://cloud.sagemath.com/projects/c293aefd-1fdf-4b9c-95f4-75bb77035e42/files/MeasuringThePriceOfAnarchyInCCUInteractions.sagews}.
