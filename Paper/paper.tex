...
In particular, the aim is to place this in a game theoretic setting so as to identify whether or not target policies result in uncoordinated behaviour that is damaging for patients.
    ...
    \begin{center}
        \begin{tikzpicture}[scale=.8, every node/.style={scale=0.8}]
            ...
            {\coordinate (origin) at (\x,0);
                % Coordinate system
                \draw [very thick, ->] (origin) -- ($(origin) + (0,5)$) node[anchor=south east] {\(\bar\lambda_{K_{\RG}}\)};
                \draw [very thick, ->] (origin) -- ($(origin) + (5,0)$) node[anchor=west] {Occupancy at RG};
                % lambda
...
Based on the previous observation this in turn implies:

\begin{equation}
    U_{\NH}(K_{\RG}) = U_{\NH}(\bar \lambda_{\RG}(K_{\RG}) \geq U_{\NH}(\bar \lambda_{\RG}(K_{\RG}+1)) = U_{\NH}(K_{\RG}+1)
    \label{equ:UNH_as_a_function_of_KRG}
\end{equation}

In the same way (illustrated by Figure \ref{fig:effect_of_KNH_on_arrival_rate}), this gives:

\begin{equation}
    U_{\NH}(K_{\NH}) = U_{\NH}(\bar \lambda_{\NH}(K_{\NH}) \leq U_{\NH}(\bar \lambda_{\NH}(K_{\NH}+1)) = U_{\NH}(K_{\NH}+1)
...
\begin{figure}[!hbtp]
    \begin{center}
        \begin{tikzpicture}[scale=.8, every node/.style={scale=0.8}]
            ...
            {\coordinate (origin) at (\x,0);
                % Coordinate system
                \draw [very thick, ->] (origin) -- ($(origin) + (0,5)$) node[anchor=south east] {\(\bar\lambda_{K_{\NH}}\)};
                \draw [very thick, ->] (origin) -- ($(origin) + (5,0)$) node[anchor=west] {Occupancy at NH};
                % lambda
...
\begin{figure}[!hbtp]
    \begin{center}
        \begin{tikzpicture}[scale=.8, every node/.style={scale=0.8}]
            % Coordinate system
            \draw [very thick, ->] (0,0) -- (0,6) node[anchor=south east] {\(K_{\RG}\)};
            \draw [very thick, ->] (0,0) -- (7,0) node[anchor=north west] {\(K_{\NH}\)};
        ...
        \end{tikzpicture}
    \end{center}
    \caption{Values \(U_{\NH}\).}\label{fig:variation_of_UNH}
\end{figure}
...
Let \(f^+(K_{\RG})=\argmin_{\{0\leq k \leq C_{\NH}|U_{\NH}(k, K_{\RG})\geq t\}}(U_{\NH}(k, K_{\RG}))\) and \(f^-(K_{\RG})=\argmax_{\{0\leq k \leq C_{\NH}|U_{\NH}(k, K_{\RG})\leq t\}}(U_{\NH}(k, K_{\RG}))\).
So that \(f^{\pm}({K_{\RG}})\) is the value of \(K_{\NH}\) that gives the value of \(U_{\NH}(K_{\NH}, K_{\RG})\) that is closest to \(t\) such that:
...
Note that from (\ref{equ:UNH_as_a_function_of_KNH}) have \(\max S(K_{\RG})=f^{+}(K_{\RG})\) and \(\min S(K_{\RG})=f^{+}(K_{\RG})\).
...
\begin{enumerate}
    \item \(S(K_{\RG})\ne S(K_{\RG})\)
    \item \(S(K_{\RG})= S(K_{\RG})\)
\end{enumerate}

If \(S(K_{\RG})\ne S(K_{\RG})\) this implies that \(f^{+}(K_{\RG})\leq f^{-}(K_{\RG})\) which gives:
...
To finish the proof consider \(S(K_{\RG})= S(K_{\RG})\) and assume:

\begin{equation}
    f(K_{RG})=f^{+}(K_{\RG}) \text{ but } f(K_{RG}+1)=f^{-}(K_{\RG}+1)
\end{equation}

which contradicts the required result.

For simplicity of notation let:

\[a^{\pm}=(f^{\pm}(K_{\RG}), K{\RG})\text{ and }b^{\pm}=(f^{\pm}(K_{\RG}+1), K{\RG}+1)\]
