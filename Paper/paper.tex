...
In \cite{A} an empirical study is undertaken and service rate slow down is identified.
Furthermore, it is shown that modelling whilst ignoring these state dependent rates leads to errors.
This effect is further identified in \cite{B,C} where for example the negative effect on patient outcome is revealed.
In \cite{D,E} this is expanded to consider a variety of admission policies in two CCUs, in particular the effect of rejection (due to too high occupancy) is measured.
...
In practice a simplification of rational strategies is managed by policy makers by setting utilisation targets that ensure that hospitals do not run at a level likely to have a high rejection rate (example of these can be seen \cite{Bevan2006, url1, url2}).
