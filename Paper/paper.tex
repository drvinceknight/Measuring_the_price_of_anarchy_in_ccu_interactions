...
%As $K_{RG}$ increases $\left(U_{NH}(k^*)-t\right)^2$ decreases until $\left(U_{NH}(k^*)-t\right)^2$ is minimized and then increases, however $\left(U_{NH}(k^*+1)-t\right)^2$ will continue to decrease.
